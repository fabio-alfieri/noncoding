...
for (mutation_type in mutation_types) {
  ...
  mclapply(tumor_types, mc.cores = cores, function(tumor_type){
    ...
    if(mutation_type == "removeWGD"){
      ...
      scna <- scna[scna$patient_id %in% substr(absolute[absolute$`Genome doublings` == 0,]$sample, 1, 12),]
      snv_filt <- snv_filt[snv_filt$patient_id %in% substr(absolute[absolute$`Genome doublings` == 0,]$sample, 1, 12),]
    }
    ...
    if (mutation_type == "expressed010") {
      ...
      tpm <- tpm[[tumor_type]]
      tpm <- tpm[tpm$median > quantile(tpm$median, prob = 0.1),]
      snv_filt <-
